...
public class BehaviorMecanim : MonoBehaviour
{
    ...
    #region Dance
    public Node Node_Dance(Val<string> gestureName, Val<bool> start)
    {
        return new LeafInvoke(
            () => this.Character.BodyAnimation(gestureName, start),
            () => this.Character.BodyAnimation(gestureName, false));
    ...
    #endregion
    #region Fight
    public Node Node_Fight(Val<Vector3> target, Val<bool> start)
    {
        ST_TurnToFace(target);
        return new LeafInvoke(
            () => this.Character.BodyAnimation("FIGHT", start),
            () => this.Character.BodyAnimation("FIGHT", false));
    }
    public Node Node_RandomDeath(Val<Vector3> target, Val<bool>start)
    {
        ST_TurnToFace(target);
        return new LeafInvoke(
            () => this.Character.BodyAnimation("DYING", start),
            () => this.Character.BodyAnimation("DYING", false));
    }
    public Node Node_WTF(Val<Vector3> target, Val<bool>start)
    {
        ST_TurnToFace(target);
        return new LeafInvoke(
            () => this.Character.BodyAnimation("DUCK", start),
            () => this.Character.BodyAnimation("DUCK", false));
    }
    #endregion
